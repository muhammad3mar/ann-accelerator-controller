...
package input_buffer_pkg;
    ...
    //--------------------------------------------------------------------------
    // Data Organization Constants (primary knobs)
    //--------------------------------------------------------------------------
    ...
    // Derived from MNIST dimensions
    localparam int MNIST_PIXELS      = MNIST_ROWS * MNIST_COLS;   // 64
    localparam int BUFFER_SIZE       = MNIST_PIXELS;              // Input-buffer depth
    localparam int BUFFER_ADDR_WIDTH = $clog2(BUFFER_SIZE);       // 6 for 64 entries
    localparam int BUFFER_DATA_WIDTH = 8;
    localparam int BUFFER_MAX_ADDR   = BUFFER_SIZE - 1;           // 63
    ...
    // Address ranges for each row (8×8 image laid out linearly in buffer)
    ...
    // Column offset (0-7) from buffer address lower bits
    ...
    // Lower nibble of a buffer word (weight 0)
    ...
    // Upper nibble of a buffer word (weight 1)
