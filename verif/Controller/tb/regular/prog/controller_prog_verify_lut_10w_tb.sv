...
// and max re-PROG attempts (prog_retry_cnt hits MAX_PROG_RETRIES) -> ERASE then TB releases under-drive.
//
...
module controller_prog_verify_lut_10w_tb;
    ...
    // After max re-PROG, first VERIFY->ERASE goes to ERASE; then release under-drive so PROG+VERIFY can pass.
    ...
    //   5: under-program until prog_retry_cnt exhausts -> ERASE (max tries) then release under-drive
    always_comb begin
        ...
        if (current_weight_idx >= 0 && in_verify_phase) begin
            ...
            else if (current_weight_idx == 5 && !w5_release_under && weight_matrix[5] > 0)
                weight_read_data_mock = actual_from_ann - 1;
        end
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            ...
            w5_release_under <= 1'b0;
        else if (dut.state == S_ERASE && prev_dut_state == S_VERIFY)
    ...
    initial begin
        ...
        $fdisplay(fd, "// idx0 equal | idx1 read< | idx2 read> | idx3-4,6-9 happy | idx5 max re-PROG (prog_retry_cnt) -> ERASE");
        $fdisplay(fd, "//------------------------------------------------------------------------------");
